...
In this step, a new empty repository was created on GitHub. This repository will act as the central location where all project history and updates will be stored. Creating a remote repository is essential for enabling synchronization between the local machine and GitHub.
![Remote repo](a.png)
...
A local Java project was created using NetBeans. This includes setting up the folder structure and generating the initial Java class. These files are not yet tracked by Git.
...
The program was extended to generate random values using Java's Random class. This prepares the project for later sorting operations.
![Random](g.png)

## h) Commit changes
These new modifications were committed to maintain a clear record.
![Commit](h.png)
...
## j) Commit changes
Once again, the code changes were staged and committed.
![Commit](j.png)
...
## o) Revert changes
The git revert command was used to reverse past changes by creating a new commit that negates a previous one.
```
...
The sorting method was improved inside the feature branch.
![Improved code](x.png)
...
A deliberate conflict was created by editing the same lines of code.
![Conflict](z1.png)
